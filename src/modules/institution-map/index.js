...
export async function initInstitutionMap(container) {
  ...
  const mapYearSlot = container.querySelector('.map-year-slot');
  ...
  try {
    ...
    function getFilteredNodes() {
      ...
      if (start <= minYear && end >= maxYear) return nodes;
      currentYear = start === end ? String(start) : `${start}-${end}`;
      return nodes.filter((n) => Number(n.year) >= start && Number(n.year) <= end);
    }
    ...
    if (mapYearSlot) mapYearSlot.appendChild(yearFilter.element);

    // initial population uses all years
    currentYear = 'all';
    recomputeForYear();
    ...
    function renderPoints() {
      ...
      statEl.textContent = `可见机构 ${visible.length}/${institutions.length} · 年份 ${(() => { const { start, end } = yearFilter.getRange(); return start === end ? String(start) : `${start}—${end}`; })()} · 归一化别名 ${aliasLookup.size} 个 · 联系 ${instLinks.length} 条`;
      renderScenario();
      renderLinks();
      renderRanking();
      renderLegend();
